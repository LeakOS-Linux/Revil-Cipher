...
void decrypt_file(const std::string& filepath, const SecByteBlock& key) {
    try {
        ...
        // Hapus file terenkripsi jika berhasil didekripsi
        fs::remove(filepath);
        std::cout << "[✓] Pulihkan: " << original_path << std::endl;

    } catch (const std::exception& e) {
        // Abaikan error pada file yang sedang digunakan sistem
    }
}
...
void scan_and_decrypt(const std::string& path, const SecByteBlock& key) {
    ...
    // Gunakan try-catch per folder agar tidak berhenti jika satu folder akses dilarang
    try {
...
SecByteBlock load_and_decrypt_aes_key() {
    ...
    if (!f_enc) throw std::runtime_error("File aes_key.enc tidak ditemukan!");
    ...
    try {
        d.Decrypt(prng, 
            reinterpret_cast<const byte*>(enc_aes_key.data()), enc_aes_key.size(), 
